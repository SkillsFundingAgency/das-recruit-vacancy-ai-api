...
public class LLMExec(IVacancyQA qa, IOptions<VacancyAiConfiguration> configuration) : ILLMExec
{
    ...
    private async Task GetCheckLlmResult(string? input,
        ConcurrentBag<ErrorReturnObject> llmerrors, ConcurrentBag<AICheckOutput> aichecksShortlist, string checkName, Prompt prompt,float temperature=1.0F)
    {
        var llmOutput = await qa.CallLLM(
            ...
            input ?? " ",
            checkName,
            temperature // configurable temperature passthru
        );
